...
function logic_reward() {
  ...
  payout--;
  credits++;
  cred_p.innerHTML = "Calories (" + credits + ")";
...
function updateAutoSpinButton() {
  ...
  if (isAutoSpinning) {
    autoSpinButton.style.backgroundColor = "red";
    autoSpinButton.innerHTML = "Stop Auto Spin";
    document.getElementById("spinType").disabled = true;
  } else {
    autoSpinButton.style.backgroundColor = "green";
    autoSpinButton.innerHTML = "Auto Spin";
    document.getElementById("spinType").disabled = false;
  }
}

function autoSpin() {
  isAutoSpinning = !isAutoSpinning; // Toggle the auto-spin state

  if (isAutoSpinning) {
    var spinType = document.getElementById("spinType").value;
    intervalId = setInterval(function() {
      spin(spinType); // Spin with the selected number of lines
    }, 1000); // Adjust the interval (in milliseconds) as per your preference
  } else {
    clearInterval(intervalId);
  }

  updateAutoSpinButton(); // Update the button appearance
}

// Call updateAutoSpinButton() once to initialize the button appearance
